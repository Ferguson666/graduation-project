...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=40,
                 _batch_size=10, generalized=False, ratio=0.7, epoch=20):
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                loss.backward()
                self.optimizer.step()
            # first_all_output = torch.empty(7924,12)
            # first_all_pred = torch.empty(7924)
       #     netA = model.ChannelAttention(self)
       #     self.test_unseen_feature = netA( self.test_unseen_feature.reshape(245,2,2,512))
       #     self.test_unseen_feature = self.test_unseen_feature.reshape(245,2048)
        ...
        self.train_Y = torch.cat((self.backup_Y.float(), first_easy_pred.float()), 0)
        ...
        knn_all_pred = torch.cat((first_easy_pred.long(), knn_hard_pred.long()), 0)
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                loss.backward()
                self.optimizer.step()
            acc, pred, output, _ = self.val(self.test_unseen_feature[keep_indices],
            ...
            # print(correct_num, all_length )
            first_acc = (correct_num / all_length).float()
    ...
    def fit(self):
        ...
        first_acc = 0
        first_all_pred = None
        #    first_all_pred =torch.empty(625)
        first_all_output = None
        ...
        for epoch in range(self.nepoch):
            ...
            acc_seen, pred_seen, output_seen = self.val_gzsl(self.test_seen_feature, self.test_seen_label,
                                                             self.seenclasses)
            acc_unseen, pred_unseen, output_unseen = self.val_gzsl(self.test_unseen_feature, self.test_unseen_label,
                                                                   self.unseenclasses)
    ...
    #   print('Seen: {:.2f}%, Unseen: {:.2f}%, H: {:.2f}%'.format(acc_first_seen * 100, acc_first_unseen * 100,
      #                                                            acc_first_H * 100))
        ...
        entropy_value = torch.from_numpy(np.asarray(list(map(entropy, first_all_output.data.cpu()))))
        #   entropy_value = entropy_value .cpu()
        #  entropy_value = entropy_value.numpy()
        _, indices = torch.sort(-entropy_value)
        ...
        self.train_Y = torch.cat((self.backup_Y.long(), first_easy_pred.long()), 0)
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            ...
            if torch.sum(idx) == 0:
                ...
                acc_per_class += torch.sum(test_label[idx].long() == predicted_label[idx].long()).float() / torch.sum(
                    idx).float()
        acc_per_class /= target_classes.size(0)
        return acc_per_class

    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        acc_per_class = torch.FloatTensor(nclass).fill_(0.0)
        for i in range(nclass):
            idx = (test_label == i)
            if torch.sum(idx) != 0:
                acc_per_class[i] = torch.sum(test_label[idx].long() == predicted_label[idx].long()).float() / torch.sum(
                    idx).float()
        acc_per_class = acc_per_class.float()
