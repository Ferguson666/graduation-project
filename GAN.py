...
parser.add_argument('--dataset', default='XG', help='XG')
parser.add_argument('--dataroot', default='E:/test3.6', help='path to dataset')
parser.add_argument('--matdataset', default=True, help='Data in matlab format')
parser.add_argument('--image_embedding', default='res095')
parser.add_argument('--class_embedding', default='att95_plus')
parser.add_argument('--syn_num', type=int, default=14, help='number features to generate per class')
...
parser.add_argument('--attSize', type=int, default=14, help='size of semantic features')
...
parser.add_argument('--netG_name', default='MLP_2HL_Dropout_G')
parser.add_argument('--netD_name', default='MLP_2HL_Dropout_CRITIC')
...
parser.add_argument('--nclass_all', type=int, default=11, help='number of all classes')
parser.add_argument('--ratio', type=float, default=0.3, help='ratio of easy samples')
parser.add_argument('--proto_param1', type=float, default=0.0001, help='proto param 1')
parser.add_argument('--proto_param2', type=float, default=0.0001, help='proto param 2')
parser.add_argument('--loss_syn_num', type=int, default=300, help='number of real clusters')
...
print('Params: dataset={:s}, GZSL={:s}, cls_weight={:.4f}, proto_param1={:.4f}, proto_param2={:.4f}'.format(
    opt.dataset, str(opt.gzsl),opt.cls_weight,opt.proto_param1, opt.proto_param2))
sys.stdout.flush()
...
def generate_syn_feature_with_grad(netG, classes, attribute, num):
    ...
    return syn_feature.cpu(), syn_label.cpu()
...
pretrain_cls = classifier.CLASSIFIER(data.train_feature, util.map_label(data.train_label, data.seenclasses),
                                     data.seenclasses.size(0), opt.resSize, opt.cuda, 0.001, 0.5, 50, 10,
...
for epoch in range(opt.nepoch):
    ...
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        labels = Variable(input_label.view(opt.batch_size, 1).cuda())
        real_proto = Variable(data.real_proto.cuda())
        fake = fake.cuda()
      #  print(fake.device, labels.device)
        ...
        #seen_feature = seen_feature.cuda()
       # print(seen_feature.device)
        ...
        seen_feature = Variable(seen_feature.cuda())
       # print(transform_matrix.device)
        ...
        #print(G_cost.device,c_errG.device,loss1.device,loss2.device)
        errG.backward()
        optimizerG.step()
    ...
    if opt.gzsl:
     ...
     #   re_feature = syn_feature.reshape(syn_feature.size(0), 2, 2, 512).cuda()
      #  syn_feature_e = netA(re_feature)
      #  syn_feature = syn_feature_e.reshape(syn_feature.size(0), 2048)
        train_X = torch.cat((data.train_feature.cpu(), syn_feature.cpu()), 0)
        train_Y = torch.cat((data.train_label, syn_label), 0)
        nclass = opt.nclass_all
        cls = classifier2.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr,0.5,100, opt.syn_num ,True)
     #print('unseen=%.4f, seen=%.4f, h=%.4f' % (cls.acc_unseen, cls.acc_seen, cls.H))
        print ("识别新冠肺炎的广义零样本学习准确率:")
        print('Seen: {:.2f}%, Unseen: {:.2f}%, H: {:.2f}%'.format(cls.acc_seen * 100, cls.acc_unseen * 100,cls.H * 100))
    # Zero-shot learning
    else:
        syn_feature, syn_label = generate_syn_feature(netG, data.unseenclasses, data.attribute, opt.syn_num)
        re_feature = syn_feature.reshape(syn_feature.size(0), 2, 2, 512).cuda()
        syn_feature_e = netA(re_feature)
        syn_feature = syn_feature_e.reshape(syn_feature.size(0), 2048)
        cls = classifier2.CLASSIFIER(syn_feature.cpu(), util.map_label(syn_label, data.unseenclasses), data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, 0.5, 40, opt.syn_num,False, opt.ratio, epoch)
